...
class GuiController:
    ...
    def __init__(self, model, gui, settings):
        self._model = model
        self._gui = gui
    ...
    def _set_valid_inputs(self):
        ...
        self._settings.settings["Face Width"].setPlaceholderText("5~24.99 (cm)")
        self._settings.settings["Face Width"].setValidator(QDoubleValidator(5, 24.99, 2))  # bottom, top, decimals
        self._settings.settings["Face Width"].setMaxLength(5)

        self._settings.settings["Distance"].setPlaceholderText("10~99.99 (cm)")
        self._settings.settings["Distance"].setValidator(QDoubleValidator(10, 99.99, 2))
        self._settings.settings["Distance"].setMaxLength(5)
    ...
    def _load_configs(self):
        ...
        for parameter, line_edit in self._settings.settings.items():
    ...
    def _connect_actions(self):
        ...
        self._settings.action_buttons["Confirm"].clicked.connect(self._confirm)
    ...
    def _check_inputs_validity(self):
        ...
        for parameter_line in self._settings.settings.values():
            if not parameter_line.hasAcceptableInput():
                check_passed = False
                parameter_line.clear()
                self._settings.warning_label.setText("error: parameter out of range")

        if check_passed:
            # Clear the message label to make sure no previous error.
            self._settings.warning_label.clear()
    ...
    def _set_model_parameters(self):
        ...
        if self._gui.options["Distance Measure"].isChecked():
            self._model.enable_distance_measure(
                True,
                face_width=float(self._settings.settings["Face Width"].text()),
                distance=float(self._settings.settings["Distance"].text())
            )
        else:
