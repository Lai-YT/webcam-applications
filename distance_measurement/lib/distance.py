...
def face_data(image: ndarray) -> Tuple[int, Union[ndarray, Tuple], Tuple[int, int]]:
    ...
    faces: Union[ndarray, Tuple] = (
        face_detector.detectMultiScale(gray_image, 1.3, 5))  # ndarray[int, int, int, int]
    face_width: int = 0
...
def draw_face_area(image: ndarray, faces: ndarray, distance_level: int,
                   *, call_out: bool = False) -> None:
    '''Draw Rectangle and display the distance over Screen.
    @param
      image: simply the frame
      face:
        upper-left x, upper-left y, height of face, width of face
    ...
    for (x, y, h, w) in faces:
        ...
        if call_out:
            # cv2.line(image, (x, y), face_center, (GRAY), 1)
